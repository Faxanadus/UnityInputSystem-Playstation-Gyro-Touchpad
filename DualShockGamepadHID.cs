...
namespace UnityEngine.InputSystem.DualShock.LowLevel
{
    ...
    internal struct DualShock4HIDInputReport : IInputStateTypeInfo
    {
		...
		[FieldOffset(12)] public short gyroX;
		[InputControl(name = "gyroY",  layout = "Axis", format = "SHRT", noisy = true)]
		[FieldOffset(14)] public short gyroY;
		[InputControl(name = "gyroZ",  layout = "Axis", format = "SHRT", noisy = true)]
		...
		[InputControl(name = "touch2Y",  layout = "Integer", format = "BIT", bit = 4, sizeInBits = 12)]
		[FieldOffset(40)] public byte touchData2Mid;
		[FieldOffset(41)] public byte touchData2End;
	}
...
namespace UnityEngine.InputSystem.DualShock
{
    ...
    public class DualSenseGamepadHID : DualShockGamepad, IEventMerger, IEventPreProcessor, IInputStateCallbackReceiver
    {
        ...
        public unsafe void OnStateEvent(InputEventPtr eventPtr)
        {
            if (eventPtr.type == StateEvent.Type && eventPtr.stateFormat == DualSenseHIDInputReport.Format)
            {
				...
				byte* state = (byte*)StateEvent.FromUnchecked(eventPtr)->state;
    ...
    public class DualShock4GamepadHID : DualShockGamepad, IEventPreProcessor, IInputStateCallbackReceiver
    {
            ...
            switch (binaryData[0])
            {
                ...
                case 1:
                {
                    ...
                    *((DualShock4HIDInputReport*)stateEvent->state) = data;
					...
					//	stateEvent->state,
					//	binaryData + 1,
						//UnsafeUtility.SizeOf<DualShock4HIDInputReport>()
					//);
        ...
        // filter out three lower bits as jitter noise
		internal const byte JitterMaskLow  = 0b01110000; //expanded by one bit
		internal const byte JitterMaskHigh = 0b10001111; //expanded by one bit
		internal const byte Buttons3Mask = 0b00000011; //used to filter out gyro

		byte FilterState(byte value) { return (byte)(value >> 2); } // this filters out small shanges to the axis as to not spam the system with updates
        ...
        public unsafe void OnStateEvent(InputEventPtr eventPtr)
        {
            if (eventPtr.type == StateEvent.Type && eventPtr.stateFormat == DualShock4HIDInputReport.Format)
            {
                ...
                var actuatedOrChanged =
                    // we need to make device current if axes are outside of deadzone specifying hardware jitter of sticks around zero point
                    FilterState(newState->leftStickX) != FilterState(currentState->leftStickX) ||
					FilterState(newState->leftStickY) != FilterState(currentState->leftStickY) ||
					FilterState(newState->rightStickX) != FilterState(currentState->rightStickX) ||
					FilterState(newState->rightStickY) != FilterState(currentState->rightStickY)
        
                    || newState->leftTrigger != currentState->leftTrigger
                    || newState->rightTrigger != currentState->rightTrigger
                    || newState->buttons1 != currentState->buttons1
                    || newState->buttons2 != currentState->buttons2
					|| (newState->buttons3 & Buttons3Mask) != (currentState->buttons3 & Buttons3Mask); //filter out gyro from making device current

                if (actuatedOrChanged) 
				{
				 InputSystem.s_Manager.DontMakeCurrentlyUpdatingDeviceCurrent();
				}
				//else if ((newState->buttons3 & Buttons3Mask) == (currentState->buttons3 & Buttons3Mask)) return; //uncomment this if to completely filter out gyro
            }
            InputState.Change(this, eventPtr);
        }
        ...
        internal struct DualShock4HIDGenericInputReport
        {
            ...
            [FieldOffset(2)] public byte rightStickX;
            [FieldOffset(3)] public byte rightStickY;
            [FieldOffset(4)] public byte buttons0; //dpad & face buttons
            [FieldOffset(5)] public byte buttons1; //all other standard buttons
            [FieldOffset(6)] public byte buttons2; //System button and Touchpad button 
            [FieldOffset(7)] public byte leftTrigger;
            [FieldOffset(8)] public byte rightTrigger;
			...
			[FieldOffset(34)] public byte touchData1ID;
			[FieldOffset(35)] public byte touchData1Start;
			[FieldOffset(36)] public byte touchData1Mid;
			[FieldOffset(37)] public byte touchData1End;

			[FieldOffset(38)] public byte touchData2ID;
			[FieldOffset(39)] public byte touchData2Start;
			[FieldOffset(40)] public byte touchData2Mid;
			[FieldOffset(41)] public byte touchData2End;
            ...
            {
                var report = new DualShock4HIDInputReport
				{
					leftStickX = leftStickX,
					leftStickY = leftStickY,
					rightStickX = rightStickX,
